...
module.exports = function CommentPlugin(sequelizeDialect, settings) {
  ...
  const queryGenerator = sequelizeDialect.getQueryInterface().QueryGenerator;
  ...
  methods.forEach(function iterator(method) {
    const originalGeneratorMethod = queryGenerator[method.name];

    queryGenerator[method.name] = function proxyMethod(/* arguments */) {
      const baseFragment = originalGeneratorMethod.apply(this, arguments);
      const options = arguments[method.optionsArgument];
...
function prependComment(comment, delimiter, fragment) {
  const parts = ['/* ', sanitizeComment(comment), ' */', delimiter, fragment];
